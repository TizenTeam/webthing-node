...
function Property(thing, name, value, metadata) {
  ...
  this.validateValue = value => {
    ...
    if (this.metadata.hasOwnProperty('minimum') && value < this.metadata.minimum) {
      throw new Error(`Value less than minimum: ${this.metadata.minimum}`);
    }

    if (this.metadata.hasOwnProperty('maximum') && value > this.metadata.maximum) {
      throw new Error(`Value greater than maximum: ${this.metadata.maximum}`);
    }

    if (this.metadata.hasOwnProperty('enum') && this.metadata.enum.length > 0 && !this.metadata.enum.includes(value)) {
